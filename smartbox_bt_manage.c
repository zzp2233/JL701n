...
static int smartbox_bt_status_event_handler(struct bt_event *bt)
{
...
#endif 
 #if UTE_BT_CALL_SUPPORT 
        uteModuleCallBtSetCallingState();
#endif
    ...
    case BT_STATUS_PHONE_HANGUP:
        ...
        //升级中界面，不响应来电提醒
        #if ID_WINDOW_LANGUAGE_SWITCH
        if (UI_GET_WINDOW_ID()== ID_WINDOW_UPGRADE || UI_GET_WINDOW_ID()== ID_WINDOW_LANGUAGE_SWITCH)  //升级语言包时不弹消息提醒  ID_WINDOW_LANGUAGE_SWITCH ID_WINDOW_UPGRADE
        {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"%s,LINE=%d,OTA ING...",__func__,__LINE__);
            break;
        }
        #else
        if (UI_GET_WINDOW_ID()== ID_WINDOW_UPGRADE)  //升级语言包时不弹消息提醒  ID_WINDOW_LANGUAGE_SWITCH ID_WINDOW_UPGRADE
        {
            break;
        }
        #endif // ID_WINDOW_LANGUAGE_SWITCH
        
        UTE_MODULE_LOG(UTE_LOG_CALL_LVL,"__LINE__ =%d UI_GET_WINDOW_ID=%d ui_return_page_id=%d",__LINE__,UI_GET_WINDOW_ID(),ui_return_page_id());
        if (UI_GET_WINDOW_ID() == ID_WINDOW_CALL_DIAL||UI_GET_WINDOW_ID() == ID_WINDOW_PHONEBOOK||UI_GET_WINDOW_ID() == ID_WINDOW_CALLRECORD) 
		{
            r_printf("%s %d,win_id %d\n",__func__,__LINE__,UI_GET_WINDOW_ID());
        }
        UTE_MODULE_LOG(UTE_LOG_CALL_LVL,"ohter =%s ui_return_page_id=%d",__func__,ui_return_page_id());
        if (UI_GET_WINDOW_ID()!=0xffffffff)
        {
            UI_HIDE_CURR_WINDOW();
            if(ui_return_page_id()!=PAGE_0&&get_return_index()!=0)
            {
                UI_SHOW_WINDOW(ui_return_page_id());  
            }
            else
            {
...
static void smartbox_bt_tws_event_handler(struct bt_event *bt)
{
    ...
    case TWS_EVENT_CONNECTED:
        ...
        if (tws_api_get_role() == TWS_ROLE_MASTER) {
            //master enable
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"master do icon_open\n");
