...
static void test_send_audio_data(int init_flag)
{
    ...
    if (init_flag) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"audio send init\n");
        send_pt = 0;
        start_flag = 1;
    }
    ...
    if (send_pt >= file_size) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"audio send Complete\n");
        start_flag = 0;
        return;
    }
    ...
    while (1) {
        if (app_send_user_data_check(send_len)) {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"audio send %08x\n", send_pt);
            if (app_send_user_data(server_con_handle[cur_dev_cid], ATT_CHARACTERISTIC_ae3c_01_VALUE_HANDLE, &file_ptr[send_pt], send_len, ATT_OP_AUTO_READ_CCC)) {
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"audio send fail!\n");
                break;
            } else {
                send_pt += send_len;
            }
...
static void set_ble_work_state(u8 cid, ble_state_e state)
{
    if (state != ble_work_state[cid]) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble_work_st[%d]:%x->%x\n", cid, ble_work_state[cid], state);
...
int smbox_ble_enable_new_dev_adv(void)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"new_dev_adv\n");
    cur_dev_cid = tmp_cid;
...
void smbox_trans_cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case ATT_EVENT_HANDLE_VALUE_INDICATION_COMPLETE:
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ATT_EVENT_HANDLE_VALUE_INDICATION_COMPLETE\n");
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_ENHANCED_CONNECTION_COMPLETE: {
                ...
                if (status) {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"LE_SLAVE CONNECTION FAIL!!! %0x\n", status);
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"HCI_SUBEVENT_LE_ENHANCED_CONNECTION_COMPLETE : %0x\n", tmp_handle1);
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE: {
                ...
                rcsp_init();
#endif
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble remote rssi= %d\n", ble_vendor_get_peer_rssi(server_con_handle[cur_dev_cid]));
            }
            break;
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"HCI_EVENT_DISCONNECTION_COMPLETE_TRANS: %0x\n", packet[5]);
        ...
        case L2CAP_EVENT_CONNECTION_PARAMETER_UPDATE_RESPONSE: {
            ...
            if (tmp_cid != SMBOX_MULTI_INVAIL_INDEX) {
                ...
                if (tmp) {
                    ...
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"remoter reject!!!\n");
...
static uint16_t multi_att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"read_callback,conn_handle =%04x, handle=%04x,buffer=%08x\n", connection_handle, handle, (u32)buffer);
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"att_value_len= %d\n", att_value_len);
...
static int multi_att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"write_callback,conn_handle =%04x, handle =%04x,size =%d\n", connection_handle, handle, buffer_size);
...
static int app_send_user_data(u16 conn_handle, u16 handle, u8 *data, u16 len, u8 handle_type)
{
    ...
    if (!multi_att_get_ccc_config(conn_handle, handle + 1)) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"fail,no write ccc!!!,%04x\n", handle + 1);
    ...
    if (ret) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"app_send_fail:%d !!!\n", ret);
    }
    return ret;
}
...
static int make_set_rsp_data(void)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"rsp_data(%d):", offset);
    log_info_hexdump(buf, offset);
...
static int smbox_multi_set_adv_enable(void *priv, u32 en)
{
    ...
    if (!adv_ctrl_en) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"no open adv_ctrl_en");
        return APP_BLE_OPERATION_ERROR;
    }

    if (server_con_handle[cur_dev_cid]) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"is connect");
        return APP_BLE_OPERATION_ERROR;
    }

    if (en) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"is BLE_ST_ADV");
        next_state = BLE_ST_ADV;
    } else {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"is BLE_ST_IDLE");
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"adv_en:%d\n", en);
...
static int app_send_user_data_do(void *priv, u8 *data, u16 len)
{
    ...
    if (len < 128) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"-le_tx(%d):");
...
void smbox_bt_multi_trans_init(void)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble address:\n");
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble name(%d): %s \n", gap_device_name_len, gap_device_name);
...
{
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"rcsp_tx:%x\n", len);
#if PRINT_DMA_DATA_EN
    if (len < 128) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"-dma_tx(%d):");
        log_info_hexdump(data, len);
    } else {
        putchar('L');
    }
