...
{
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"conn_handle:%04x,report_data:%02x,%02x,%d,len(%d)", report_data->conn_handle, report_data->packet_type,
    ...
    /* if (search_uuid == NULL) { */
    /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"not_match handle"); */
    /* return; */
    /* } */

    switch (report_data->packet_type) {
    case GATT_EVENT_NOTIFICATION:  //notify
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"GATT_EVENT_NOTIFICATION");
        break;

    case GATT_EVENT_INDICATION://indicate
    case GATT_EVENT_CHARACTERISTIC_VALUE_QUERY_RESULT://read
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"GATT_EVENT_INDICATION");
        break;

    case GATT_EVENT_LONG_CHARACTERISTIC_VALUE_QUERY_RESULT://read long
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"GATT_EVENT_LONG_CHARACTERISTIC_VALUE_QUERY_RESULT");
        break;

    default:
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble_report_data_deal is default");
        break;
    }
}
...
static void client_event_callback(le_client_event_e event, u8 *packet, int size)
{
    ...
    case CLI_EVENT_MATCH_DEV: {
        ...
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"match_name:%s\n", match_dev->compare_data);
    }
    break;
    ...
    case CLI_EVENT_MATCH_UUID: {
        ...
        if (opt_hdl->search_uuid == &jl_search_uuid_table[0]) {
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"match_uuid22\n");
        }
    }
    break;

    case CLI_EVENT_SEARCH_PROFILE_COMPLETE:
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"CLI_EVENT_SEARCH_PROFILE_COMPLETE_client_event_callback\n");
        /* if ((!ble_client_timer) && ble_client_write_handle) { */
        /*     UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"test timer_add\n"); */
    ...
    case CLI_EVENT_DISCONNECT:
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"CLI_EVENT_DISCONNECT_client_event_callback");
    ...
    if (rw_flag == 0) {
        ...
        for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
            if (conn_pair_info_table[i].head_tag != BLE_VM_HEAD_TAG
                ...
                //invail pair
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"-invail,table %d!!!--\n", i);
                memset(&conn_pair_info_table[i], 0, unit_len);
            }
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"read,pair_id= %d\n", i);
        ...
        if (!info) {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"-table clean--\n");
            memset(conn_pair_info_table, 0, sizeof(conn_pair_info_table));
        } else {

            for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
                if (0 == memcmp(info, &conn_pair_info_table[i], unit_len)) {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"dev in table\n");
                    return;
                }
            }
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"find table\n");
            ...
            if (fill_index == -1) {
                for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
                    if (conn_pair_info_table[i].match_dev_id == info->match_dev_id) {
                        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"replace match_dev_id= %d\n", info->match_dev_id);
                        fill_index = i;
                        break;
                    }
                }
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"write,pair_id= %d\n", fill_index);
...
{
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"client_clear_bonding_info\n");
    //del pair bond
...
static void set_ble_work_state(u8 cid, ble_state_e state)
{
    if (state != ble_work_state[cid]) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble_client_work_st[%d]:%x->%x\n", cid, ble_work_state[cid], state);
...
static void check_target_uuid_match(search_result_t *result_info)
{
    ...
    for (i = 0; i < client_config->search_uuid_cnt; i++) {
        ...
        if (result_info->services.uuid16) {
            if (result_info->services.uuid16 != t_uuid->services_uuid16) {
                /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"b1"); */
                continue;
            }
        } else {
            if (memcmp(result_info->services.uuid128, t_uuid->services_uuid128, 16)) {
                /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"b2"); */
                continue;
            }
        }
        ...
        if (result_info->characteristic.uuid16) {
            if (result_info->characteristic.uuid16 != t_uuid->characteristic_uuid16) {
                /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"b3"); */
                /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"%d: %04x--%04x",result_info->characteristic.uuid16,t_uuid->characteristic_uuid16); */
                continue;
            }
        } else {
            if (memcmp(result_info->characteristic.uuid128, t_uuid->characteristic_uuid128, 16)) {
                /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"b4"); */
                continue;
            }
        }
    ...
    if (opt_handle_used_cnt >= OPT_HANDLE_MAX) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"opt_handle is full!!!\n");
        return;
    }

    if ((t_uuid->opt_type & result_info->characteristic.properties) != t_uuid->opt_type) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"properties not match!!!\n");
        return;
    }

    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"match one uuid\n");
...
static void do_operate_search_handle(void)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"opt_handle_used_cnt= %d\n", opt_handle_used_cnt);

    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"find target_handle:");
    ...
    for (i = 0; i < opt_handle_used_cnt; i++) {
        ...
        case ATT_PROPERTY_INDICATE:
            tmp_16  = 0x02;//fixed
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"write_ind_ccc:%04x\n", opt_hdl_pt->value_handle);
            client_operation_send(opt_hdl_pt->value_handle + 1, &tmp_16, 2, ATT_OP_WRITE);
            break;
...
{
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"slave request conn_update:\n-conn_handle= %04x\n-interval_min= %d,\n-interval_max= %d,\n-latency= %d,\n-timeout= %d\n",
             handle,
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"conn_handle:%04x,confirm_interval:%d\n", handle, cfg_interval);
#endif
...
void user_client_report_search_result(search_result_t *result_info)
{
    if (result_info == (void *) - 1) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"client_report_search_result finish!!!\n");
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"\n*** services, uuid16:%04x,index=%d ***\n", result_info->services.uuid16, result_info->service_index);
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"{charactc, uuid16:%04x,index=%d,handle:%04x~%04x,value_handle=%04x}\n",
    ...
    if (!result_info->services.uuid16) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"######services_uuid128:");
        log_info_hexdump(result_info->services.uuid128, 16);
    }

    if (!result_info->characteristic.uuid16) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"######charact_uuid128:");
...
void user_client_report_data_callback(att_data_report_t *report_data)
{
    ...
    if ((s8) - 1 == mul_get_dev_index(report_data->conn_handle, SMBOX_MULTI_ROLE_CLIENT)) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"unknown_handle:%04x\n", report_data->conn_handle);
        log_info_hexdump(report_data->blob, report_data->blob_length);
        return;
    }
...
static void client_search_profile_start(u16 conn_handle)
{
    ...
    if (tmp_cid == SMBOX_MULTI_INVAIL_INDEX) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"error search_cid!!! :%04x\n", conn_handle);
        return;
    }
    ...
    if (encryption_process == 2 && pair_reconnect_search_profile == 0) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"reconnect not search_profile:%04x\n", client_con_handle[tmp_cid]);
        user_client_set_search_complete();
        return;
    }

    if (client_config->search_uuid_cnt) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"start search_profile_all:%04x\n", client_con_handle[tmp_cid]);
        ble_op_search_profile_all();
        u8 i = ble_op_search_profile_all();
        printf("search_profile_all :%d", i);
    } else {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"skip search_profile:%04x\n", client_con_handle[tmp_cid]);
        user_client_set_search_complete();
    }
}
...
static bool resolve_adv_report(u8 *adv_address, u8 data_length, u8 *data, s8 rssi)
{
    ...
    for (i = 0; i < data_length;) {
        if (*adv_data_pt == 0) {
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"analyze end\n"); */
            break;
        }
        ...
        case HCI_EIR_DATATYPE_FLAGS:
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"flags:%02x\n",adv_data_pt[0]); */
            break;
        ...
        case HCI_EIR_DATATYPE_COMPLETE_128BIT_SERVICE_UUIDS:
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"service uuid:"); */
        ...
        case HCI_EIR_DATATYPE_SHORTENED_LOCAL_NAME:
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ble remoter_name: %s,rssi:%d\n", adv_data_pt, rssi);
...
#endif
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"target name:%s", client_config->compare_data); */
            if (check_device_is_match(CLI_CREAT_BY_NAME, adv_data_pt, lenght - 1)) {
                find_remoter = 1;
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"catch name ok\n");
            }
            break;

        case HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA:
            if (check_device_is_match(CLI_CREAT_BY_TAG, adv_data_pt, lenght - 1)) {
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"get_tag_string!\n");
                find_remoter = 1;
            }
            break;

        case HCI_EIR_DATATYPE_APPEARANCE_DATA:
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"get_class_type:%04x\n",little_endian_read_16(adv_data_pt,0)); */
            break;

        default:
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"unknow ad_type:"); */
            break;
        }
...
static void client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    u8 pair_cnt = 0;
    int i;
    /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"event_type,addr_type: %x,%x; ",report_pt->event_type,report_pt->address_type); */
    /* log_info_hexdump(report_pt->address,6); */

    /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"adv_data_display:"); */
    /* log_info_hexdump(report_pt->data,report_pt->length); */

    //    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"rssi:%d\n",report_pt->rssi);
    /* g_printf("report_pt->address_type: %d\n",report_pt->address_type); */
    /* put_buf(report_pt->address, 6); */
    for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
        if (conn_pair_info_table[i].pair_flag) {
            pair_cnt ++;
            /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"check paired dev\n"); */
            /* put_buf(report_pt->address, 6); */
...
#endif
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"find paired dev to connect\n");
                //set tag
                find_tag = 2;
...
just_creat:
    if (find_tag) {
        if (force_seach_onoff && force_seach_rssi > report_pt->rssi) {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"match but rssi fail!!!:%d,%d\n", force_seach_rssi, report_pt->rssi);
            return;
        }

        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"rssi:%d\n", report_pt->rssi);
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"\n*********create_connection***********\n");
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"***remote type %d,addr:", report_pt->address_type);
        log_info_hexdump(report_pt->address, 6);
        ...
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"*create_finish\n");
    }
}
...
static void client_report_ext_adv_data(u8 *report_pt, u16 len)
{
    ...
    /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"--func=%s", __FUNCTION__); */
    ...
    if (find_remoter) {
        ...
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"***remote type %d, addr:", address_type);
...
static void client_ext_create_connection(u8 *conn_addr, u8 addr_type)
{
    ...
    if (get_ble_work_state(cur_dev_cid) == BLE_ST_CREATE_CONN) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"already ext_create conn:%d!!!\n", cur_dev_cid);
        return;
    }
...
static void client_create_connection(u8 *conn_addr, u8 addr_type)
{
    ...
    if (get_ble_work_state(cur_dev_cid) == BLE_ST_CREATE_CONN) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"already create conn:%d!!!\n", cur_dev_cid);
        return;
    }

    if (pair_bond_enable == 0x55) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"pair reconnect to creat\n");
...
static int client_disconnect(void *priv)
{
    ...
    if (client_con_handle[cid]) {
        if (BLE_ST_SEND_DISCONN != get_ble_work_state(cid)) {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,">>>ble(%d) send disconnect\n", cid);
            set_ble_work_state(cid, BLE_ST_SEND_DISCONN);
            ble_op_disconnect(client_con_handle[cid]);
        } else {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,">>>ble(%d) wait disconnect...\n", cid);
        }
...
static void connection_update_complete_success(u16 conn_handle, u8 *packet, u8 connect_start)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"conn_handle = %04x\n", conn_handle);
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"conn_interval = %d\n", conn_interval);
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"conn_latency = %d\n", conn_latency);
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"conn_timeout = %d\n", conn_timeout);
    ...
    if (connect_start) {
        ...
        if (conn_pair_info.conn_handle ==  conn_handle) {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"cur_conn_pair_info\n");
            conn_pair_info.conn_interval = conn_interval;
            conn_pair_info.conn_latency =  conn_latency;
            conn_pair_info.conn_timeout =  conn_timeout;
            ...
            for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
                if (conn_pair_info_table[i].pair_flag && conn_pair_info_table[i].head_tag == BLE_VM_HEAD_TAG
                    && conn_pair_info_table[i].tail_tag == BLE_VM_TAIL_TAG) {
                    //vail pair
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"%d:handle=%04x\n", i, conn_pair_info_table[i]);
                    if (conn_pair_info_table[i].conn_handle == conn_handle) {
                        ...
                        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"table_conn_pair_info:%d\n", i);
...
int ble_enable_new_dev_scan(void)
{
    ...
    if (!client_con_handle[cur_dev_cid]) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"dev_doing\n");
        return -1;
    }

    s8 tmp_cid = mul_get_idle_dev_index(SMBOX_MULTI_ROLE_CLIENT);
    if (tmp_cid == SMBOX_MULTI_INVAIL_INDEX) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"no idle dev to do!!!\n");
        return -2;
    }

    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"new_dev_scan\n");
    cur_dev_cid = tmp_cid;
...
void smbox_client_deal_passkey_input(u32 *key, u16 conn_handle)
{
    //0~999999
    *key = 123456;
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"con_handle= %04x,new_key: %06u.\n", conn_handle, *key);
}
...
void smbox_client_cbk_sm_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case SM_EVENT_JUST_WORKS_REQUEST:
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"Just Works Confirmed.\n");
        ...
        case SM_EVENT_PASSKEY_DISPLAY_NUMBER:
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"Passkey display: %06u.\n", tmp32);
            encryption_process = 1;
            break;
        ...
        case SM_EVENT_PASSKEY_INPUT_NUMBER:
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"%04x->Passkey input: %06u.\n", event->con_handle, tmp32);
            sm_passkey_input(event->con_handle, tmp32); /*update passkey*/
            break;

        case SM_EVENT_PAIR_PROCESS:
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"======PAIR_PROCESS: %02x\n", event->data[0]);
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_ENHANCED_CONNECTION_COMPLETE: {
                ...
                if (status) {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"LE_MASTER CREATE CONNECTION FAIL!!! %0x\n", status);
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"HCI_SUBEVENT_LE_ENHANCED_CONNECTION_COMPLETE : 0x%0x\n", tmp_handle1);
                s8 tmp_dev_cid1 = mul_get_idle_dev_index(SMBOX_MULTI_ROLE_CLIENT);

                if (tmp_dev_cid1 == SMBOX_MULTI_INVAIL_INDEX) {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"dev_cid full!!!\n");
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE: {
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"HCI_SUBEVENT_LE_CONNECTION_COMPLETE: %0x\n", tmp_handle);
                ...
                if (pair_bond_enable && 0 == client_config->security_en) {
                    conn_pair_info.pair_flag = 1;
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"no_pair,bond remoter\n");
                    conn_pair_vm_do(&conn_pair_info, 1);
                    pair_bond_enable = 0;
                } else {
            ...
            case HCI_SUBEVENT_LE_DATA_LENGTH_CHANGE:
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"APP HCI_SUBEVENT_LE_DATA_LENGTH_CHANGE");
                break;

            case HCI_SUBEVENT_LE_PHY_UPDATE_COMPLETE:
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"APP HCI_SUBEVENT_LE_PHY_UPDATE %s\n", hci_event_le_meta_get_phy_update_complete_status(packet) ? "Fail" : "Succ");
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"Tx PHY: %s\n", phy_result[hci_event_le_meta_get_phy_update_complete_tx_phy(packet)]);
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"Rx PHY: %s\n", phy_result[hci_event_le_meta_get_phy_update_complete_rx_phy(packet)]);
                break;

#if EXT_ADV_MODE_EN
            case HCI_SUBEVENT_LE_EXTENDED_ADVERTISING_REPORT:
                /* UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"APP HCI_SUBEVENT_LE_EXTENDED_ADVERTISING_REPORT"); */
                /* log_info_hexdump(packet, size); */
                if (BLE_ST_SCAN == get_ble_work_state(cur_dev_cid)) {
                    client_report_ext_adv_data(&packet[2], packet[1]);
                } else {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"drop ext_adv_report!!!\n");
                }
                break;
#endif /* EXT_ADV_MODE_EN */
            }
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"HCI_EVENT_DISCONNECTION_COMPLETE_client: %0x\n", packet[5]);
            ...
            if (0 == client_con_handle[cur_dev_cid]) {
                ...
                if (BLE_ST_SCAN == get_ble_work_state(cur_dev_cid) || BLE_ST_CREATE_CONN == get_ble_work_state(cur_dev_cid)) {
                    ...
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"%d,doing\n", cur_dev_cid);
                    bt_ble_scan_enable(0, 0);
                }
            }
            ...
            if (0) { //(conn_pair_info.pair_flag && SUPPORT_MAX_CLIENT < 2) {
                ...
                if (packet[5] == 0x3e) {
                    ...
                    if (app_change_key_is_press) {
                        ...
                        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"No_switch_app1!!");
                        app_change_key_is_press = 0;
                    } else {
                        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"To_switch_app1!!");
                ...
                /*     UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"Disconn_to_switch_app  :%d\n",select_app1_or_app2); */
        ...
        case ATT_EVENT_MTU_EXCHANGE_COMPLETE: {
            ...
            if (tmp_cid != SMBOX_MULTI_INVAIL_INDEX) {
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"ATT MTU = %u\n", mtu);
        ...
        case HCI_EVENT_VENDOR_REMOTE_TEST:
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"--- HCI_EVENT_VENDOR_REMOTE_TEST\n");
            break;
        ...
        case L2CAP_EVENT_CONNECTION_PARAMETER_UPDATE_RESPONSE: {
            ...
            if (tmp_cid != SMBOX_MULTI_INVAIL_INDEX) {
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"-update_rsp:%04x, %02x\n", tmp_handle, tmp);
            }
        }
        break;
        ...
        case GAP_EVENT_ADVERTISING_REPORT:
            ...
            if (BLE_ST_SCAN == get_ble_work_state(cur_dev_cid)) {
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"drop adv_report!!!%02x\n", get_ble_work_state(cur_dev_cid));
            }
            break;
        ...
        case HCI_EVENT_ENCRYPTION_CHANGE: {
            u16 tmp_handle = little_endian_read_16(packet, 3);
            s8 tmp_cid = mul_get_dev_index(tmp_handle, SMBOX_MULTI_ROLE_CLIENT);
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"HCI_EVENT_ENCRYPTION_CHANGE= %d,handle=%04x\n", packet[2], tmp_handle);

            if (packet[2]) {
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"fail,break\n");
                break;
            }

            if (0 == encryption_process) {
                encryption_process = 2;
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"reconnect remoter\n");
            } else {
                //first pair
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"first pair\n");
            }

            if (pair_bond_enable) {
                if (conn_pair_info.conn_handle == tmp_handle) {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"pair,bond remoter\n");
                    conn_pair_info.pair_flag = 1;
                    conn_pair_vm_do(&conn_pair_info, 1);
                    pair_bond_enable = 0;
                } else {
                    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"not match???\n");
                }
            }
...
static int client_operation_send(u16 handle, u8 *data, u16 len, u8 att_op_type)
{
    ...
    if (!handle) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"handle is null\n");
        return APP_BLE_OPERATION_ERROR;
    }

    if (get_buffer_vaild_len(0) < len) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"opt_buff_full!!!\n");
...
static int bt_ble_scan_enable(void *priv, u32 en)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"scan_en:%d\n", en);
...
{
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"client_init_config\n");
...
static int client_create_connect_api(u8 *addr, u8 addr_type, u8 mode)
{
    ...
    if (cur_state == BLE_ST_SCAN) {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"stop scan\n");
        bt_ble_scan_enable(0, 0);
    }
    ...
    if (mode == 1) {
        if (conn_pair_info.pair_flag) {
            if (conn_pair_info.pair_flag) {
                ...
                UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"pair to creat!\n");
            ...
            UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"no pair to creat!\n");
            return APP_BLE_OPERATION_ERROR;
        }
    } else {
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"addr to creat!\n");
...
void client_send_conn_param_update(void)
{
    ...
    UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"client update param:-%d-%d-%d-%d-\n", param->interval_min, param->interval_max, param->latency, param->timeout);
    if (client_con_handle[cur_dev_cid]) {
...
int le_multi_client_switch_connect_cid(u8 cid)
{
    if (cid < SUPPORT_MAX_CLIENT && client_con_handle[cid]) {
        ...
        UTE_MODULE_LOG(UTE_LOG_SYSTEM_LVL,"cid not exist!!!\n");
        return -1;
    }
}
